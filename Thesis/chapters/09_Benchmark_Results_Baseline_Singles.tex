...
The following results are measured on \textbf{WS-Big} unless otherwise specified. 
...
\begin{table}[H]
  \caption[Median/avg timings for frame/gpu/cull for baseline and individual optimizations]{Median and average (arithmetic mean) timings for frametime, gpu-time and cull-time for baseline and individual optimizations (rounded to two decimal places)}\label{tab:singles_medians}
  \centering
